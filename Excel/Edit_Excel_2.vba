...
Sub Edit_Book()
  ' 別ブックのシートの指定したセル範囲を編集する
  ' ブック名, シート名, セル範囲 は 決められたセルに入力して 本マクロを実行する
  ' ブックはオープンされているものとする
  ' 実行する編集機能は 番号で選択（本マクロでは 1と2 が可能）
  ' 機能によって必要なパラメータも セルに入力しておく
  ' 機能 1
  '    指定した文字列を含まない行に Outlineを設定
  ' 機能 2
  '    指定した文字列に 文字色とBold を設定
  '
  Dim wb0 As Workbook
  Dim wb1 As Workbook
